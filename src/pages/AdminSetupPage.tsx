...
interface P { onComplete: (session: AdminSession) => void; onBack: () => void; }
export function AdminSetupPage({ onComplete, onBack }: P) {
  ...
  const PinInput = ({ value, onChange, placeholder }: { value:string; onChange:(v:string)=>void; placeholder:string }) => (
    <input type="password" inputMode="numeric" pattern="[0-9]*" value={value} onChange={e=>onChange(e.target.value.replace(/\D/g,'').slice(0,8))} placeholder={placeholder}
      style={{ width:'100%', background:C.well, boxShadow:SH.downSm, border:'none', borderRadius:'10px', padding:'0.75rem', color:C.t1, fontSize:'1.2rem', fontFamily:'inherit', outline:'none', letterSpacing:'0.3em', textAlign:'center', boxSizing:'border-box' as const }} />
  );
  return <Frame><NavBar onBack={onBack}/>
    <div style={{width:'64px',height:'64px',borderRadius:'50%',background:'rgba(232,41,122,0.12)',border:'1px solid rgba(232,41,122,0.25)',boxShadow:SH.up,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 1.5rem'}}>
      <svg width="26" height="26" viewBox="0 0 24 24" fill="none"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4" stroke={C.pink} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"/></svg>
    </div>
    <div style={{textAlign:'center',marginBottom:'1.75rem'}}><div style={{fontSize:'1.2rem',fontWeight:600,color:C.t1,marginBottom:'0.35rem'}}>Create your admin account</div><div style={{fontSize:'0.75rem',color:C.t4,lineHeight:1.6}}>No admin accounts exist yet. Set up your owner account to get started.</div></div>
    <div style={{display:'flex',flexDirection:'column',gap:'1.25rem'}}>
      ...
      <div style={{background:'rgba(232,41,122,0.06)',border:'1px solid rgba(232,41,122,0.15)',borderRadius:'10px',padding:'0.75rem 1rem'}}>
        <p style={{fontSize:'0.72rem',color:C.t3,lineHeight:1.6}}>This account will be created as <span style={{color:C.pink,fontWeight:600}}>Owner</span> — you can add other admins from the Manage Access tab after setup.</p>
      </div>
